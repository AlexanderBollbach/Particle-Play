...
-(void)setupAUGraph:(MyMIDIPlayer *)player {
    ...
    hpcd.componentManufacturer = kAudioUnitManufacturer_Apple;
     ...
     hpcd.componentSubType = kAudioUnitSubType_HighPassFilter;  // changed!
...
//                "AUGraphInitialize failed");
     ...
     AUGraphAddNode(player->graph, &hpcd, &hpNode);
     AUGraphOpen(player->graph);
     AUGraphNodeInfo(player->graph, instrumentNode, NULL, &player->instrumentUnit);
     AUGraphNodeInfo(player->graph, reverbNode, NULL, &player->reverbUnit);
     AUGraphNodeInfo(player->graph, hpNode, NULL, &player->hpUnit);
     AUGraphConnectNodeInput(player->graph, instrumentNode, 0, reverbNode, 0);
     AUGraphConnectNodeInput(player->graph, reverbNode, 0, hpNode, 0);
     AUGraphConnectNodeInput(player->graph, hpNode, 0, outputNode, 0);
...
-(void) sendNoteOnEvent:(UInt32)key velocity:(UInt32)velocity {
     ...
     MusicDeviceMIDIEvent (_player.instrumentUnit,
                           (int)(midiNote+midiOutChannel),
                           key & 0x7F,
                           velocity & 0x7F,0);
}

-(void) sendNoteOffEvent:(UInt32)key velocity:(UInt32)velocity {
     MusicDeviceMIDIEvent (_player.instrumentUnit,(int)(128+0),key & 0x7F,velocity & 0x7F,0);
}

-(void) sendNoteCCEvent:(UInt32)channel data:(UInt32)value {
     MusicDeviceMIDIEvent (_player.instrumentUnit,(int)(0+176),channel & 0x7F ,value & 0x7F,0);
}

-(void)setHighPassFreqCutoff:(AudioUnitParameterValue)value {
     AudioUnitSetParameter(_player.hpUnit, kAudioUnitScope_Global, 0, kLowPassParam_CutoffFrequency, value, 0);
}
...
- (id)initWithPresetPath:(NSString *)path {
    ...
    if (self) {
        ...
        CheckError (AUGraphStart(_player.graph),"couldn't start graph");
          ...
          AudioUnitSetParameter(_player.hpUnit, kAudioUnitScope_Global, 0, kDelayParam_Feedback, 0.5, 0);
         AudioUnitSetParameter(_player.hpUnit, kAudioUnitScope_Global, 0, kDelayParam_WetDryMix, 0, 0);
    }
    return self;
}
